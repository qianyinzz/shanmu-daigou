...
/**
 * 集成 Vite 开发服务器（中间件模式）
 * 使用动态 import 避免 tsup 打包时拉入 Vite 及其 native 依赖链
 */
export async function setupViteMiddleware(app: Application) {
  ...
  const vite = await createViteServer({
    ...(viteConfig.default as Record<string, unknown>),
    configFile: false,
    server: {
      ...((viteConfig.default as Record<string, unknown>).server as Record<string, unknown>),
      middlewareMode: true,
    },
    appType: 'spa',
  });
